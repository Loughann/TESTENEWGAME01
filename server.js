...
app.post('/api/webhook/vizzionpay', async (req, res) => {
  try {
    ...
    if (!gatewayTxId || amountCents <= 0) {
      return res.json({ received: true, note: "Data invalid" });
    }
    ...
    if (checkTxRes.ok) {
      const existing = await checkTxRes.json();
    ...
    let phone = clientEmail.includes('@blockcash.com') ? clientEmail.split('@')[0].trim() : '';
    ...
    if (!profile && clientEmail) {
      ...
      if (profEmailRes.ok) {
        const profs = await profEmailRes.json();
        if (profs && profs.length > 0) profile = profs[0];
      }
    }
    ...
    if (!profile) {
      console.error(`[Express Webhook] Usuário não encontrado para e-mail: ${clientEmail} ou ID: ${gatewayTxId}`);
      return res.json({ received: true, userNotFound: true });
    }

    phone = profile.phone;

    // Atualização de Saldo & Bônus de Primeiro Depósito
    ...
    console.log(`[Express Webhook] Sucesso! Depósito creditado para ${phone}: R$ ${amount.toFixed(2)} + Bônus R$ ${(bonusCents/100).toFixed(2)}. Saldo: R$ ${(newBalanceCents/100).toFixed(2)}`);
